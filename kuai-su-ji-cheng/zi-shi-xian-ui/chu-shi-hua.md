...
初始化 SDK 前，您需要初始化一个 [`LogLevelEnum`](../../api/enums/loglevelenum.md) 对象。该对象用于对 SDK 进行日志级别设置。
...
我们建议您在调用initSDK时传入 [`V2TimSDKListener`](../../api/guan-jian-lei/listener/v2timsdklistener.md) 接口添加 SDK 事件监听器，在对应回调中做一些逻辑处理。
...
### 调用初始化接口
操作完上述步骤后，您可以调用 [`initSDK`](../../api/v2timmanager/initsdk.md) 进行 SDK 初始化。
...
反初始化需要操作一个步骤：调用反初始化接口 [`unInitSDK`](../../api/v2timmanager/uninitsdk.md)
