...
## 消息搜索类介绍
### 消息搜索参数类
消息搜索参数类为 [`V2TimMessageSearchParam`](../../../api/keyClass/message/v2timmessagesearchparam.md)。搜索消息时，SDK 会按照该对象的设置，执行不同的搜索逻辑。
...
### 消息搜索结果类
消息搜索结果类为 [`V2TIMMessageSearchResult`](../../../api/keyClass/message/v2timmessagesearchresult.md)。参数说明如下：
...
其中 `messageSearchResultItems` 是个列表，内含 [`V2TIMMessageSearchResultItem`](../../../api/keyClass/message/v2timmessagesearchresultitem.md) 对象，参数说明如下：
...
如果您希望在全部会话范围内搜索，只需要将 [`V2TimMessageSearchParam`](../../../api/keyClass/message/v2timmessagesearchparam.md) 中的 `conversationID` 设置为空（`null`/`nil`) 或者不设置即可。
...
如果不需要离线推送可以用发消息接口 [`sendMessage`](../../../api/v2timmessagemanager/sendmessage.md) 的参数 [`V2TIMOfflinePushInfo`](../../../api/keyClass/message/v2timofflinepushinfo.md) 中的 `disablePush` 来控制。
