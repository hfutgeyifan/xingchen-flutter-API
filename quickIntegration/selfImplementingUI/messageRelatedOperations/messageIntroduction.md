## 消息类介绍
在腾讯云 IM SDK 中，消息类为 [`V2TIimessage`](../../../api/keyClass/message/v2timmessage.md) 。您在发送和接收消息过程中都会频繁地用到该类。
...
| nickName | 消息发送者的昵称 | 客户自己设置。调用 [`setSelfInfo`](../../../api/v2timmanager/setselfinfo.md) 设置及修改。详情可参考 [用户资料](../usersRelatedOperations/usersInformation.md)。|
...
| mergerElem | 合并消息存储元素 | 发送合并位置消息需要创建并填充该元素；收到消息判断 elemType 为 MERGER 时，可以从 mergerElem 中解析出内容。详情可参考 [转发消息](../../../api/v2timmessagemanager/createforwardmessage.md) |
| groupTipsElem | 群 tips 消息存储元素 | 群 tips 消息目前只能由服务器发送。SDK 收到消息判断 elemType 为 GROUP_TIPS 时，可以从 groupTipsElem 中解析出内容。详情可参考 [接收消息](../../../api/callbacks/onrecvnewmessagecallback.md) |
