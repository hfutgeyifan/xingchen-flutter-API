## 功能描述
* 通过 [`addAdvancedMsgListener`](../../../api/v2timmessagemanager/addadvancedmsglistener.md) 监听接收所有类型消息（文本、自定义、富媒体消息），相关回调在 [`V2TimAdvancedMsgListener`](../../../api/keyClass/listener/v2timadvancedmsglistener.md) 协议中定义。
...
1. 调用 [`addAdvancedMsgListener`](../../../api/v2timmessagemanager/addadvancedmsglistener.md) 设置事件监听器。
2. 监听 [`onRecvNewMessage`](../../../api/callbacks/onrecvnewmessagecallback.md) 回调，在其中接收自定义消息。
3. 希望停止接收消息，调用 [`removeAdvancedMsgListener`](../../../api/v2timmessagemanager/removeadvancedmsglistener.md) 移除监听。该步骤不是必须的，客户可以按照业务需求调用。
...
1. 接收方调用 [`addAdvancedMsgListener`](../../../api/v2timmessagemanager/addadvancedmsglistener.md) 接口设置高级消息监听。
2. 接收方通过监听回调 [`onRecvNewMessage`](../../../api/callbacks/onrecvnewmessagecallback.md) 获取消息 V2TimMessage。
3. 接收方解析 V2TIMMessage 消息中的 elemType 属性，并根据其类型进行二次解析，获取消息内部 Elem 中的具体内容。
4. 希望停止接收消息，调用 [`removeAdvancedMsgListener`](../../../api/v2timmessagemanager/removeadvancedmsglistener.md) 移除监听。该步骤不是必须的，客户可以按照业务需求调用。
...
接收到地理位置消息后，接收放可直接从 [`locationElem`](../../../api/keyClass/message/v2timlocationelem.md) 中解析出经纬度信息。
...
SDK 仅为表情消息提供消息透传的通道，消息内容字段参考 [`faceElem`](../../../api/keyClass/message/v2timfaceelem.md) 定义。其中 `index` 和 `data` 的内容由客户自定义。
...
群 tips 消息有多种类型，参考 [`V2TIMGroupTipsType`](../../../api/enums/grouptipselemtype.md) 的定义。

接收方接受到群 tips 消息后，一般需要做：
1. 解析出 [`V2TIMGroupTipsElem`](../../../api/keyClass/group/v2timgrouptipselem.md) 中的各个字段
