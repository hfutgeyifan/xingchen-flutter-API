...
|   | 适用场景 |
|---------|---------|
| [使用DEMO](#第三部分使用demo) | IM Demo 是一个完整的聊天 App，代码已开源，如果您需要实现聊天类似场景，可以使用 Demo 进行二次开发。 可立即 [体验 Demo](demoExperience.md)。 |
...
- `--dart-define=KEY={YOUR_KEY}` 其中`{YOUR_KEY}`需替换成 [第一部分：创建测试用户](#part1) 中的`密钥（Key
{% endhint %}
...
## 第四部分：含UI集成，使用TUIKit组件库，半天完成IM能力植入

TUIKit 是基于腾讯云 IM SDK 的一款 UI 组件库，它提供了一些通用的 UI 组件，例如会话列表、聊天界面和联系人列表等，开发者可根据实际业务需求通过该组件库快速地搭建自定义 IM 应用。参见 [TUIKit 详细介绍](../xingchen-flutter-API/quickIntegration/tuikit/fastIntegrationSolution.md)。
...
如您需要用到推送能力，还需要添加推送相关权限，详情可查看 [Flutter 厂商消息推送插件集成指南](../xingchen-flutter-API/quickIntegration/selfImplementingUI/offlinePush.md)。
{% endhint %}
...
[TIMUIKitContact](uikit-sdk-api/TIMUIKitContact/README.md): 联系人列表页面。

[TIMUIKitGroupProfile](uikit-sdk-api/TIMUIKitGroupProfile/README.md): 群资料页面，使用方式与 `TIMUIKitProfile` 基本一致。

[TIMUIKitGroup](uikit-sdk-api/TIMUIKitGroup/README.md): 群列表界面。

[TIMUIKitBlackList](uikit-sdk-api/TIMUIKitBlackList/README.md): 黑名单列表界面。

[TIMUIKitNewContact](uikit-sdk-api/TIMUIKitNewContact/README.md): 联系人（好友）申请列表。如需在外部显示小红点，可使用 `TIMUIKitUnreadCount` 小红点组件，其会自动挂载监听。

[TIMUIKitSearch](uikit-sdk-api/TIMUIKitSearch/README.md): 搜索组件，支持全局搜索联系人/群组/聊天记录，也支持在特定会话中搜索聊天记录。两种模式取决于是否传入 `conversation`。

详细UI插件指南可参见 [本文档](uikit-sdk-api/README.md) 或 [插件 README](https://pub.dev/packages/tim_ui_kit)。
...
[本节详细文档](quickIntegration/selfImplementingUI/integratedSDK.md)
...
[本节详细文档](quickIntegration/selfImplementingUI/initialization.md)
...
在本步骤，你可以针对 IM SDK 挂载一些监听，主要包括网络状态及用户信息变更等，详情可参见 [该文档](api/v2timmanager/initsdk.md)。

#### 登录测试账户

[本节详细文档](quickIntegration/selfImplementingUI/loginAndlogout.md)
...
[本节详细文档](quickIntegration/selfImplementingUI/messageRelatedOperations/sendMessage.md)
...
[本节详细文档](quickIntegration/selfImplementingUI/conversationRelatedOperations/conversationList.md)
...
[本节详细文档](quickIntegration/selfImplementingUI/messageRelatedOperations/receiveMessage.md)
...
您可以继续完成 [群组](quickIntegration/selfImplementingUI/groupsRelatedOperations/groupsIntroduction.md)，[用户资料](quickIntegration/selfImplementingUI/usersRelatedOperations/usersInformation.md)，[关系链](quickIntegration/selfImplementingUI/usersRelatedOperations/friendsManagement.md)，[离线推送](quickIntegration/selfImplementingUI/offlinePush.md)，[本地搜索](quickIntegration/selfImplementingUI/localSearch/searchMessage.md) 等相关功能开发。

详情可查看 [自实现 UI 集成 SDK 文档](quickIntegration/selfImplementingUI/README.md)。

## 常见问题

### 支持哪些平台？
- 目前 [IM SDK(tencent_im_sdk_plugin)](quickIntegration/selfImplementingUI/integratedSDK.md) 支持 iOS 、Android 和 Web 三个平台，此外 Windows 和 Mac 版正在开发中，敬请期待。
- [TUIKit](quickIntegration/tuikit/README.md) 及 [配套完整版交互 Demo](https://github.com/TencentCloud/TIMSDK/tree/master/Flutter/Demo/im-flutter-uikit) 支持 iOS 及 Android 两个移动平台。
